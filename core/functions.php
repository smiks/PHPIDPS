...
function checkLimitGet(){
	global $_GET;
	return sizeof($_GET);
}

function checkLimitPost(){
	global $_POST;
	return sizeof($_POST);
}

/* functions for checking white and black lists */
function checkWhiteListGet(){
...
function detectSuspiciousDatasctructureGet(){
	global $_GET;
...
function detectSuspiciousDatasctructurePost(){
	global $_POST;
...
function detectCharactersGet(){
	global $_GET, $illegalCharacters;

	foreach ($_GET as $key => $value) {
		foreach ($illegalCharacters as $chr) {
			if(is_array($key)){
				$tmp = "";
				foreach ($key as $k => $v) {
					$tmp .= $k." ".$v;
...
function detectCharactersPost(){
	global $_POST, $illegalCharacters;
...
$onEvents = ["onafterpring", "onbeforeprint", "onbeforeunload", "onerror", "onhashchange", 
...
function detectJSOnEventsGet(){
	global $_GET, $onEvents;

	foreach ($_GET as $key => $value) {
		foreach ($onEvents as $event) {
...
function detectJSOnEventsPost(){
	global $_POST, $onEvents;
...
function detectXSSGet(){
	global $_GET, $XSSpatterns;
...
function detectXSSPost(){
	global $_POST, $XSSpatterns;
...
function detectSQLinjectionGet(){
	global $_GET, $SQLpatterns;
...
function detectSQLinjectionPost(){
	global $_POST, $SQLpatterns;
...
function detectCSRF(){
	global $_POST, $_GET, $_originURL, $_SERVER;
	if((isset($_POST) || isset($_GET)) && isset($_SERVER['HTTP_REFERER']) && parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST) != $_originURL){
		return true;
	}
	return false;
}